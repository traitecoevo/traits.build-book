...
austraits <- austraits::load_austraits(doi = "10.5281/zenodo.8353840") # XXX need to update to version 5.0.0 for tutorial functions to work
```
...
*How many taxon have leaf N data in AusTraits? How are these data distributed across datasets*
...
*How many "hydraulic" traits are in AusTraits? How much data exists for these traits?*
...
As such, you can use `dplyr::join` functions to join together the relational tables into a single (much) wider table. Some `{austraits}` package functions also join key columns to the traits table.
...
| taxonomic_updates |                                                                                                                                                 | dataset_id, taxon_name, original_name                                                                                                                          |
| contributors      |                                                                                                                                                 | dataset_id                                                                                                                                                     |
...
-   [**summarise and lookup functions**](#summarise_functions): These functions offer summaries by taxon name or trait, summarising taxa per trait (or variable), datasets per trait, and observations per trait.

-   [**filtering functions**](#filtering_functions): These functions begin with the work `extract` and filter all of the relational tables simultaneously.
...
The advantage of the `{austraits}` package functions is that they simultaneously subset both the core traits table and all the ancillary data tables.

However, there are many columns within the traits tables that you cannot (currently) filter from using `{austraits}` functions.
...
For many research purposes you will want to join metadata from one of the relational tables to the core traits table. There are a series of `{austraits}` functions that facilitate this - or you can use `{dplyr} left_join` functions to join tables.
...
To add additional location properties you need to use `dplyr::left_join` in conjunction with manipulations to the locations table.

1.    The number of location properties is long. You'll first want to select specific location properties to merge onto the traits table:
...
While the `{austraits}` function offers a quick method for merging on pre-selected data, becoming familiar with how you can select the specific location properties you want to merge using `tidyr::pivot_wider` and `dplyr::left_join` offers the flexibility to tailor your join request to the exact information relevant to your study.

#### Joining contexts

The context tables documents any additional measurements of non-traits that may be useful for interpreting trait values. They are dividing into 5 categories, treatment context, plot context, entity context, temporal context, and method context, distinguishing between the entity on which the measurement is made.
...
# next chunk of code to be removed after function is edited - XX
...
As this function adds columns to the traits table for all context properties, you may prefer to filter the contexts table to only include specific context properties:
...
tmp$contexts <- tmp$contexts %>%
  dplyr::filter(context_property %in% c("sampling season", "plant sex", "leaf surface", "leaf age", "fire intensity", "slope position", "fire season", "drought treatment", "temperature treatment"))

traits_with_contexts_fewer <- 
  tmp %>% austraits::join_contexts()
```

#### Joining methods

The methods table documents a selection of metadata recorded about the entire dataset and methods used for individual trait measurements. There is a single row of data per `dataset_id` * `trait_name` * `method_id` combination. `Method_id` is used to distinguish between instances where a single trait is measured using two separate protocols and is separate to `method_context_id` which documents explicit differences in a method between measurements. 
...
The function `austraits::summarise_trait_means()` outputs `location` * `taxon_name` * `dataset_id` means, but is not (*currently*) appropriately updating/removing columns such as `observation_id`, `individual_id`, and `replicates`.
...
data$traits %>%
  filter(!is.na(`latitude (deg)`)) %>%        # reduce the size of the dataframe by only retaining values with geographic coordinates
...
data$traits %>% data$traits %>% 
  filter(!is.na(`latitude (deg)`)) %>%
  austraits::plot_locations("taxon_name")     # 1 taxon, so 1 plot

data$traits %>% 
  filter(!is.na(`latitude (deg)`)) %>%
  austraits::plot_locations("dataset_id")     # 1 plot for each dataset_id
```
...
The AusTraits tables are all in `long` format with an individual row for each trait measurement. This is the most compact way to store data and offers the flexibility of documenting different metadata values for each measurement. 
