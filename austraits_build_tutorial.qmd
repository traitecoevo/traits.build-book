...
data %>% austraits::plot_locations("genus")
```
...
traits_with_contexts <- 
  (austraits %>% austraits::join_contexts())$traits %>% 
...
```{r, eval = FALSE}
traits_with_methods <- 
  (austraits %>% austraits::join_methods())$traits
```
...
```{r, eval = FALSE}
austraits_with_taxa <-
  (austraits %>% austraits::join_taxonomy())$traits
```
...
data %>% austraits::plot_locations("dataset_id")
...
austraits$traits %>%
  ...
  dplyr::filter(original_name != taxon_name) %>% head()
