...
```{r, eval = FALSE}
> austraits$traits %>% filter(trait_name == "leaf_N_per_dry_mass") %>% distinct(taxon_name) %>% nrow()
[1] 2262
...
austraits$traits %>% filter(trait_name == "sapwood_specific_hydraulic_conductivity") %>% distinct(taxon_name) %>% nrow()
[1] 179
...
data %>% austraits::plot_locations("trait_name")
```
...
data %>% austraits::plot_locations("taxon_name") # 1 taxon, so 1 plot
data %>% austraits::plot_locations("dataset_id") # 1 plot for each dataset_id
```
...
data <-
  (austraits %>%
     ...
     austraits::join_locations %>% austraits::join_taxonomy)$traits

data %>% plot_locations("genus")
```
...
austraits$traits %>% 
  filter(trait_name %in% N_traits) %>% 
  distinct(taxon_name, trait_name) %>% 
  mutate(counter = 1) %>% 
  group_by(trait_name) %>% 
    mutate(counter = sum(counter)) %>% 
    distinct(trait_name, counter) %>% 
  ungroup()
```
...
leaf_mass_per_area_values <-
  austraits %>% 
     extract_trait(trait_names = c("leaf_mass_per_area"))
...
my_data <-
  austraits$traits %>%
    filter(stringr::str_detect(taxon_name, "Eucalyptus")) %>%
    filter(trait_name %in% c("leaf_mass_per_area", "leaf_length", "leaf_thickness")) %>%
...
traits_with_locations <- 
  austraits$traits %>% 
  left_join(locations, by = c("dataset_id", "location_id"))
```
...
locations <- 
  austraits$locations %>%
  filter(location_property %in% c("latitude (deg)", "longitude (deg)", "description", "aridity index (MAP/PET)", "soil type", "fire history")) %>%
  pivot_wider(names_from = location_property, values_from = value)
...
Joining the context table to the traits table is best done using the austraits.R function, as there are 5 identifiers in the traits table (`entity_context_id`, `method_context_id`, `temporal_id`, `plot_id` and `treatment_id`) that link to two columns in the contexts table (`link_vals` and `link_id`).

```{r, eval = FALSE}

traits_with_contexts <- 
  (austraits %>% join_contexts)$traits %>% 
  select(-all_of(c("temporal_id", "method_id", "treatment_id", "entity_context_id", "plot_id")))  # "..._id" to become "..._context_id" in v5.0.0
```
...
traits_with_all_methods <- 
  austraits$traits %>% 
  left_join(austraits$methods %>% 
              distinct(dataset_id, trait_name, .keep_all = TRUE),    # to deal with a soon to be fixed bug
            by = c("dataset_id", "trait_name"))                      # a reworked "method_id" will be added here soon
```
...
traits_with_all_methods <- 
  austraits$traits %>% 
  left_join(austraits$methods %>% 
              select("dataset_id", "trait_name", "methods", "description",  "source_primary_citation", "data_collectors") %>% 
              distinct(dataset_id, trait_name, .keep_all = TRUE),      # to deal with a soon to be fixed bug
            by = c("dataset_id", "trait_name"))                        # a reworked "method_id" will be added here soon
```
...
data <-
  (austraits %>%
     extract_trait(trait_names = c("leaf_mass_per_area", "plant_height")) %>%
     join_locations)$traits
...
data %>% plot_locations("dataset_id")

data <-
  (austraits %>%
     extract_dataset("Schulze_2014") %>%
     join_locations)$traits
...
You can summarise all of AusTraits by trait name:

```{r, eval = FALSE}
austraits::summarise_austraits(austraits, "trait_name")
austraits::summarise_austraits(austraits, var =  "family")
austraits::summarise_austraits(austraits, var =  "genus")
```
...
austraits$traits %>% 
  filter(trait_name %in% N_traits) %>% 
  distinct(dataset_id, trait_name) %>% 
  mutate(counter = 1) %>% 
  group_by(trait_name) %>% 
    mutate(counter = sum(counter)) %>% 
    distinct(trait_name, counter) %>% 
...
categorical_summary_wider <- 
  categorical_summary_by_value(austraits, "resprouting_capacity") %>%
    pivot_wider(names_from = value_tmp, values_from = replicates)
```
...
Edwards_2000_values <-
  austraits %>%
     extract_dataset("Edwards_2000")

Edwards_2000_pivoted <- Edwards_2000_values$traits %>% trait_pivot_wider

View(Edwards_2000_pivoted)
```

Notice that the values at the individual, population and species level do not collapse together. This is because traits measured on different `entity_types` have separate `observation_id`'s.
...
```{r, eval = FALSE}
Edwards_2000_pivoted_merged <- merge_entity_types("Edwards_2000")
```
...
```{r, eval = FALSE}
Westoby_2014_pivoted_merged <- merge_entity_types("Westoby_2014")
```
...
```{r, eval = FALSE}
austraits$traits %>%
  filter(stringr::str_detect(taxon_name, "Eucalyptus sp\\.$")) %>%
  distinct(dataset_id, taxon_name, original_name) %>%
...
```{r, eval = FALSE}
austraits$traits %>%
  filter(stringr::str_detect(original_name, "Dryandra")) %>%
  distinct(taxon_name, original_name) %>%
  filter(original_name != taxon_name)
...
```{r, eval = FALSE}
austraits$traits %>%
     filter(stringr::str_detect(taxon_name, "Argyrodendron sp. Whyanbeel")) %>%
     distinct(taxon_name, original_name)
