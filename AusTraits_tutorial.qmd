...
Before you begin, source essential packages and functions.
...
This tutorial uses the most recent AusTraits release, version `r austraits::get_versions()[[2]][1]`

```{r, eval = TRUE}
most_recent <- austraits::get_versions()[[3]][1]
...
Moreover, as there are currently `r length(unique(austraits$locations$location_property))` location properties, you'' want to first filter the locations properties, to tailor your join request to the exact information relevant to your study.
