# AusTraits tutorial {#sec-AusTraits_tutorial}

## Introduction

With more than 1.8 million data records, AusTraits is Australia's [largest plant trait database](austraits_database.qmd#plant_database), created using the [`{traits.build}` R package](https://github.com/traitecoevo/traits.build)
...
::: {.callout-note}
This chapter is the **extended, worked walkthrough** of exploring and wrangling data, using AusTraits
as the example database. The [`austraits` package chapter](austraits_package.qmd#sec-austraits_package) is a more concise
tour of the same functions — `extract_`, `join_`, `summarise_`, plotting, and pivoting — and applies
to any `traits.build` database, not only AusTraits. Read that chapter for a quick reference; read this
one for step-by-step examples.
:::
...
Then download (or build) the latest AusTraits database, using one of the methods described [here](austraits_database.qmd#access_data).
...
The database is output as a list, a collection of relational tables, described in detail [here](database_structure.qmd#sec-database_structure).
...
## Exploring AusTraits {#austraits_functions}
...
The [`austraits` package chapter](austraits_package.qmd#sec-austraits_package) covers these functions as well.
...
-   [**pivot functions**](#pivotting_datasets): These functions allow the traits table to be pivoted to wide format.
...
See the [database structure chapter](database_structure.qmd#sec-database_structure) for names and definitions of each column and, for those with controlled vocabulary, their allowed values.
...
## Citing the data you use {#citing_data}

When you extract a subset of a database for an analysis, you should cite the **original sources** the
data came from, not just the database as a whole. Because `traits.build` databases retain their full
structure through the `extract_` functions, the provenance you need travels with your subset.

To see the citations for the sources in a subset, look at the `methods` table, which carries a primary
citation for every dataset:

```{r, eval = FALSE}
subset_data <- austraits %>% extract_dataset(c("Falster", "Wright", "Westoby", "White"))

subset_data$methods %>%
  dplyr::distinct(dataset_id, source_primary_citation)
```

To produce a BibTeX file you can import into a reference manager (Zotero, EndNote, Mendeley, …), run
`RefManageR::WriteBib()` on the subset's `sources` element:

```{r, eval = FALSE}
# writes a .bib file of all sources in the subset to your working directory
RefManageR::WriteBib(subset_data$sources)

# or write to a named file
RefManageR::WriteBib(subset_data$sources, file = "austraits_sources.bib")
```

You can also inspect or export a single source:

```{r, eval = FALSE}
subset_data$sources["Falster_2005_1"]
```

This is the most reliable way to acknowledge everyone whose data you have used — for example, to find
the full citation behind the values for a particular trait, extract that trait and read off the
citations from its `methods` table or exported `.bib`.
...
### More complex workflows -- some examples {#sample_analyses}

#### An example looking at trait-climate gradients

A simple workflow allows one to look at [trait values across a climate gradient](traits_and_climate_example.qmd#sec-traits_and_climate_example)
...
[Alternatives to box plots](https://labs.ala.org.au/posts/2023-08-28_alternatives-to-box-plots/)

We've adopted it [here](spatial_data_example.qmd#sec-spatial_data_example).
...
The [AusTraits Plant Dictionary (APD)](https://w3id.org/APD/) offers detailed descriptions for all trait concepts included in AusTraits. With the APD, each trait is given a unique, resolvable identifier, allowing trait definitions to be reused and shared.
