# Tutorial 1: Adding a simple dataset {#sec-tutorial_dataset_1}
...
Before you begin this tutorial, ensure you have installed traits.build, cloned the traits.build-template repository, and have successfully build a database from the datasets in `traits.build-template` . Instructions are available at [Tutorial: Example compilation](tutorial_compilation.qmd#sec-tutorial_compilation).

### Goals

-   Learn how to [build a metadata.yml file](#build_metadata) for a dataset.

-   Learn how to [merge a new dataset](#build_pipeline) into a `traits.build` database.
...
-   You can add multiple data collectors by duplicating the relevant 4 lines of code; see the [adding data chapter](adding_data_long.qmd#sec-adding_data_long) for protocols on who to add as a data collector.
...
| methods        | See the study's [test_dataset_1_notes.txt](https://github.com/traitecoevo/traits.build-template/blob/master/data/tutorial_dataset_1/raw/test_dataset_1_notes.txt) file | A verbatim (free-form) text field documenting the methods used to collect the trait measurements. This is generally available from the reference or directly from the author.                                            |
