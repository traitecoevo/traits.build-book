---
aliases:
  - ../file_organisation.html
---

# File organisation {#sec-file_organisation}
...
# NB: no `full_width = TRUE` here. In latex mode kableExtra renders that as a
# `tabu` environment; tabu is unmaintained and errors out against current
# LaTeX ("Undefined control sequence \tabu@cleanup"). `scale_down` already
# fits the table to the text block, so full_width added nothing.
...
The file `traits.yml` provides the trait definitions used to compile the trait database, including allowable trait values. See [creating a trait dictionary](create_dictionary.qmd#sec-create_dictionary) for more information on the process of creating this file. A `.yml` file is a structured data file where information is presented in a hierarchical format (see [appendix for details](yaml.qmd#sec-yaml)).
...
```{r, results='show'}
# NB: `freeze: auto` keys on this .qmd, not on the CSV, and the CI cache key
# hashes only `**/*qmd`. So editing data/taxon_list.csv alone reuses the stale
# frozen output and the change never reaches the built book — this chapter has
# to be touched too to invalidate its freeze entry.
