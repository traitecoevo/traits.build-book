# The `austraits` package {#sec-austraits_package}
...
austraits <- load_austraits(version = "7.0.0", path = "data/austraits")
```

`7.0.0` is the current release at the time of writing; run `get_versions()` (below) to see what the
...
subset_multi_studies <- extract_dataset(database = austraits,
                                        dataset_id = c("Westoby_2014","Wright_2019"))
