...
To create a single database for distribution to the research community, we developed a reproducible and transparent workflow in R for merging each dataset into a traits.build database. The pipeline ensures the following information is standardised across all datasets within a database. A `metadata` file for each study documents how the `data tables` submitted by an individual contributor are translated into the standardised terms used within the database. 

* **taxonomic nomenclature** should follow an agreed-upon standard for the specific database. For instance, AusTraits follows the Australian Plant Census (APC), and the R package `APCalign` offers a workflow to correct minor spelling mistakes, indicate which scientific names are current versus outdated and align with a known genus when a full species names isn't provided. If a reference exists that matches synonyms to currently accepted/valid names, the traits.build workflow will automatically update names during processing.
* **trait names** are defined in an accompanying `traits.yml` file and only data for traits included in this file can be merged into the database. The trait names used in the incoming dataset are mapped onto the appropriate trait name from the dictionary. 
* For **numeric traits** the `traits.yml` file includes `units` and the allowable `range` of values. All incoming data are converted to the appropriate units and data outside the range of allowable values are removed from the main database data table.
* For **categorical traits** the `traits.yml` file includes a list of allowable `values`, allowed terms for the trait. Each categorical trait value is defined in the `traits.yml` file. Lists of substitutions translate the exact syntax and terms in a submitted dataset into the values specified within the database trait dictionary. This ensures that for a certain trait the same `value` has an identical meaning throughout the database.
...
We consider detailed error checking to be an important and ongoing part of our workflow. The following steps are taken to ensure data quality.

* The data curator can run a series of **tests** on each data set, detailed in the [adding data chapter](adding_data_long.qmd#sec-adding_data_long)
...
* Each dataset is then compiled into a **report** which summarises metadata and plots/charts trait values in comparison to other measurements of that trait in the database. The report is reviewed by the data contributor to ensure metadata is complete and data values are as expected.
* A second member of the database team double checks each dataset before it is merged into the main repository.
