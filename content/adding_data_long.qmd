...
# Adding datasets, a lengthy guide {#sec-adding_data_long}
...
This chapter is an exhaustive reference for adding datasets to a traits.build database.

If you are embarking on building a new database using the `traits.build` standard a better place to get started are 7 [tutorials](tutorial_dataset_1.qmd#sec-tutorial_dataset_1).
...
-   an [overview of `traits.build`](traits_build.qmd#sec-traits_build),
-   the [structure of a compiled `traits.build` database](database_structure.qmd#sec-database_structure),
-   the [structure of the raw data files](file_organisation.qmd#sec-file_organisation)
-   the [overview for adding data](adding_data_brief.qmd#sec-adding_data_brief).
...
### Good practices when collecting data {#good_data_collection}

Much of the effort of adding a dataset disappears if the data were recorded tidily in the first place.
If you are collecting data yourself, or advising students setting up spreadsheets for data entry, a
few habits make a dataset far easier to bring into `traits.build` (and to reuse generally):

-   **Use consistent names across every table.** A location or taxon should be spelled and formatted
    identically everywhere it appears — in the trait table, the location table, and any notes. If
    field sheets use short codes or abbreviations, keep a small **lookup table** that maps each code to
    its full name, rather than scattering variants that later have to be reconciled by hand.
-   **Keep taxon names in one place and one form.** Record the name as identified, and avoid mixing
    authorities, abbreviations, and misspellings; taxonomic alignment is much smoother when the
    original names are internally consistent.
-   **Record core metadata explicitly**, either as a separate table or as dedicated columns — who
    collected the data, where, when, and under what conditions. Don't leave this implicit in a file
    name or a colleague's memory.
-   **Always capture dates.** Include the collection date (and, where relevant, the analysis date) in
    the data spreadsheet itself. `yyyy-mm-dd` is ideal, but any consistent format can be parsed later.
-   **Record units unambiguously.** Put the unit in the trait's column header (e.g.
    `leaf_area_mm2`) or in a separate table that documents the unit for each trait — never leave units
    to be guessed. Keep one measurement per cell.
-   **Keep the rawest values possible.** Record individual measurements rather than pre-computed means
    where you can, and note the number of replicates. Summarising is easy later; recovering lost detail
    is not.

These conventions map directly onto the `data.csv` structure below, so data collected this way needs
little reshaping before it can be compiled.
...
As described in detail [here](workflow.qmd#sec-workflow) the `metadata.yml` file maps the meanings of the individual columns within the `data.csv` file and documents all additional dataset metadata.
...
Cross-dataset duplicates are *flagged* by hand and *filtered out* here. There is no generalised check yet, so identifying which values were also sourced from a different study is currently a case-by-case judgement -- see [Duplicate measurements](data_common_issues.qmd#duplicates), and note that some cross-dataset overlap is legitimate rather than a resubmission. Once you have identified genuine duplicates, remove them with `custom_R_code` and record in the dataset's `notes` field which values you removed and why. Functions to automate the detection step are in progress.
...
-   **value_type**: Value type indicates the statistical nature of the trait value recorded. Allowable value types are `mean`, `minimum`, `maximum`, `mode`, `range`, `raw`, and `bin`. See the [database structure chapter](database_structure.qmd#value_types) for definitions of these accepted value types. All categorical traits are generally scored as being a `mode`, the most commonly observed value. Note that for values that are `bins`, the two numbers are separated by a double-hyphen, `1 -- 10`.

-   **basis_of_value**: Basis of value indicates how a value was determined. Allowable terms are `measurement`, `expert_score`, `model_derived`, and `literature`. See the [database structure chapter](database_structure.qmd#value_types) for definitions of these accepted `basis_of_value` values, but most categorical traits measurements are values that have been scored by an expert (`expert_score`) and most numeric trait values are `measurements`.
...
-   **methods**: This information can usually be copied verbatim from a manuscript and is a textual description of all components of the method used to measure the trait.
  ...
  In general, methods sections extracted from pdfs include "special characters" (non-UTF-8 characters). Non-English alphabet characters are recognised (e.g. é, ö) and should remain unchanged. Other characters will be re-formatted during the study input process, so double check that degree symbols (°), en-dashes (--), em-dashes (---), and curly quotes (',',",") have been maintained or reformatted with a suitable alternative. Greek letters and some other characters are replaced with their Unicode equivalent (e.g. \<U+03A8\> replaces Psi (Ψ)); for these it is best to replace the symbol with an interpretable English-character equivalent.

::: {.callout-warning}
## The degree symbol has a look-alike

The degree symbol must be `°` (U+00B0 DEGREE SIGN). Text copied from PDFs and word processors very often carries `º` (U+00BA MASCULINE ORDINAL INDICATOR) instead, which is near-indistinguishable in most fonts but is a disallowed character — it is the single most common one across the AusTraits databases. `dataset_replace_disallowed_chars()` repairs it; see [Running tests](#running_tests).
:::
...
For instance, the `write_metadata` function re-serialises the file with `yaml::as.yaml()`, which rewraps long lines and reworks line breaks (except in custom_R_code). Any change you see to degree symbols, dashes, quotes or more obscure symbols is a side effect of that serialisation and escaping — `write_metadata` holds no list of character substitutions and applies none deliberately. In particular, **do not expect the read/write cycle to repair a disallowed character**: a wrong degree symbol will survive it untouched. To fix those, see [Running tests](#running_tests).
...
Certain special characters show up as errors. `dataset_replace_disallowed_chars()` repairs the ones with a known replacement — including the degree symbol look-alike described above. It defaults to a dry run, so you can read the report before anything is written:

```{r, eval=FALSE, echo=TRUE}
# Report what would change, without writing
traits.build::dataset_replace_disallowed_chars(current_study)

# Apply the replacements
traits.build::dataset_replace_disallowed_chars(current_study, dry_run = FALSE)
```

By default it touches only `metadata.yml`. Characters with no known replacement — accented names and legitimate typography, for instance — are listed in the report and left alone, so those still need adjusting by hand in the `metadata.yml` file.
...
See the [common issues](data_common_issues.qmd#sec-data_common_issues) chapter for solutions to common issues, such as:  
-   dataset not [pivoting](data_common_issues.qmd#cannot_pivot)  
...
To continue on to [building the dataset report](#report), you need to rebuild the entire database:
