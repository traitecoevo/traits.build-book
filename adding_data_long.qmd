...
This vignette is an exhaustive reference for adding datasets to a traits.build database. If you are embarking on building a new database using the `traits.build` standard a better place to get started are 7 [tutorials](tutorial_dataset_1.html). Then come back to this document for details and unusual dataset circumstances that are not covered in the tutorials. 
...
```{r, eval=FALSE, echo=TRUE}
metadata_add_source_doi(dataset_id = current_study, doi = "doi", type = "secondary")
```
