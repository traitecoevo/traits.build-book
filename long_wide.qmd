# Long vs Wide data {#sec-long_wide}
...
Two {tidyr} functions, `pivot_longer` and `pivot_wider` make it quick to convert data from `long` to `wide` format in R. The [AusTraits tutorial](AusTraits_tutorial.qmd#sec-AusTraits_tutorial) offers multiple examples for how these functions can be used to pivot the various relational tables in a traits.build database, including the [core traits table](AusTraits_tutorial.qmd#pivotting_datasets).
